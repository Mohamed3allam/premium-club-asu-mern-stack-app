...
const UserPage = () => {
    ...
    const { userAccessed, dispatchUsers } = useUsersContext()
    ...
    const handleUpdateData = async (e) => {
        ...
        const response = await fetch(`${premiumApi}/users/edit-user/${userAccessed._id}`, {
            method:'PUT',
            body: updatedData,
            headers: {
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            const response = await fetch(`${premiumApi}/users/user/${userId}`, {
                method:'GET',
                headers:{
                    'Authorization': `Bearer ${user.token}`
                }
            ...
            const json = await response.json()
            if (response.ok) {
                dispatchUsers({ type: 'SET_USER', payload: json})
            }
        }
        user && fetchUserData()
