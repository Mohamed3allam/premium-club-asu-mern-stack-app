...
const Nav = styled.nav`
    ...
    padding: 0 9%;
`
const ClientNavbar = ({committees, images}) => {
    ...
    const [committeess, setCommittees] = useState(false)
            ...
            <div className="collapse navbar-collapse" id="navbarSupportedContent">
                <ul className="navbar-nav me-auto mb-2 mb-lg-0  ml-auto mr-5">
                    ...
                    <li className="nav-item dropdown">
                        <Link className={`nav-link dropdown-toggle ${committeess && 'active'}`} to="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                            Our Committees
                        </Link>
                        {
                            committees && (
                                <ul className="dropdown-menu" aria-labelledby="navbarDropdown">
                                    {
                                        committees.map((committee) => (
                                            <div key={committee._id}>
                                                <li><Link className={`dropdown-item committeeSelector`} to={`/committees/${committee.committee_name.replace(/\s+/g, '-').toLowerCase()}`}>{committee.committee_name}</Link></li>
                        ...
                        user && (
                            <li className='nav-item dropdown' style={{display: 'flex', flexDirection: 'row'}}>
                                <Link className="nav-link dropdown-toggle" to="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false" style={{display: 'flex', flexDirection:'row',alignItems:'center'}}>
                                    <img src={PPisLoading ? images['defaultPP.jpg'] : (premiumApi + user.user.profilePicUrl)} id="profilePic" style={{width:'40px', height:'40px',borderRadius:'50%',objectFit: 'cover',border:'solid gray 2px',marginRight:'6%',backgroundImage:`url('${images['defaultPP.jpg']}')`}}/>
